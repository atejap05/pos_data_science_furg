...
def show_plots(history, plot_title=None, fig_size=None):
    ...
    with sns.axes_style("darkgrid"):
        ...
        f, ax = plt.subplots(nrows=1, ncols=2, figsize=((16, 5) if fig_size is None else fig_size))
...
def save_keras_model_json(model, base_file_name, save_dir=os.path.join('.','model_states')):
...
def load_keras_model_json(base_file_name, load_dir=os.path.join('.', 'keras_models'),
                          use_tf_keras_impl=True):
